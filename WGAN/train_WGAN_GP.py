...
out_directory = './wgan_gp_out/'
model_directory = './wgan_gp_model/'
...
def generator(z,batch_size = batch_size,phase_train=True,reuse = False):
    ...
    with tf.variable_scope('gen', reuse=reuse):
        ...
        g_4 = tf.nn.leaky_relu(g_4,leak_value)
...
def trainGAN():
    ...
    difference = G_sample - X
    interpolates = X + (alpha*difference)
    gradients = tf.gradients(discriminator(interpolates),[interpolates])[0]
    slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients),reduction_indices=[1]))
    gradien_penalty = tf.reduce_mean((slopes-1.)**2)
    D_loss += LAMBDA * gradien_penalty

    D_solver = tf.train.AdamOptimizer(learning_rate=d_lr, beta1=0.5, beta2=0.9).minimize(-D_loss,var_list=theta_D)
    G_solver = tf.train.AdamOptimizer(learning_rate=g_lr, beta1=0.5, beta2=0.9).minimize(G_loss,var_list=theta_G)
    ...
    for it in range(iterations):
        ...
        if it % 400 == 0:
            ...
            np.save(out_directory + '/' + str(it) + '_model.npy', samples)

            if not os.path.exists(model_directory):
                os.makedirs(model_directory)
            saver.save(sess,save_path=model_directory + '/' +str(it) + '.cptk')
