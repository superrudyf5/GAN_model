...
def generator(z,batch_size = batch_size,phase_train=True,reuse = False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        g_1 = tf.nn.conv3d_transpose(z, weights['wg1'], (batch_size, 3, 3, 3, 256), strides=[1, 1, 1, 1, 1],
                                     padding="VALID")
        g_1 = tf.nn.bias_add(g_1, biases['bg1'])
        g_1 = tf.contrib.layers.batch_norm(g_1, is_training=phase_train)
        g_1 = tf.nn.relu(g_1)

        g_2 = tf.nn.conv3d_transpose(g_1, weights['wg2'], (batch_size, 6, 6, 6, 128), strides=strides, padding="SAME")
        g_2 = tf.nn.bias_add(g_2, biases['bg2'])
        g_2 = tf.contrib.layers.batch_norm(g_2, is_training=phase_train)
        g_2 = tf.nn.relu(g_2)

        g_3 = tf.nn.conv3d_transpose(g_2, weights['wg3'], (batch_size, 12, 12, 12, 64), strides=strides,
                                     padding="SAME")
        g_3 = tf.nn.bias_add(g_3, biases['bg3'])
        g_3 = tf.contrib.layers.batch_norm(g_3, is_training=phase_train)
        g_3 = tf.nn.relu(g_3)

        g_4 = tf.nn.conv3d_transpose(g_3, weights['wg4'], (batch_size, 24, 24, 24, 1), strides=strides, padding="SAME")
...
def initialiseWeights():
    ...
    weights['wg1'] = tf.get_variable("wg1", shape=[3, 3, 3, 256, 200], initializer=xavier_init)
    weights['wg2'] = tf.get_variable("wg2", shape=[3, 3, 3, 128, 256], initializer=xavier_init)
    weights['wg3'] = tf.get_variable("wg3", shape=[3, 3, 3, 64, 128], initializer=xavier_init)
    weights['wg4'] = tf.get_variable("wg4", shape=[3, 3, 3, 1, 64], initializer=xavier_init)
    # weights['wg5'] = tf.get_variable("wg5", shape=[4, 4, 4, 1, 64], initializer=xavier_init)

    weights['wd1'] = tf.get_variable("wd1", shape=[3, 3, 3, 1, 64], initializer=xavier_init)
    weights['wd2'] = tf.get_variable("wd2", shape=[3, 3, 3, 64, 128], initializer=xavier_init)
    weights['wd3'] = tf.get_variable("wd3", shape=[3, 3, 3, 128, 256], initializer=xavier_init)
    weights['wd4'] = tf.get_variable("wd4", shape=[3, 3, 3, 256, 1], initializer=xavier_init)
...
def initialiseBiases():
    ...
    biases['bg2'] = tf.get_variable("bg2", shape=[128], initializer=zero_init)
    biases['bg3'] = tf.get_variable("bg3", shape=[64], initializer=zero_init)
    ...
    biases['bd4'] = tf.get_variable("bd4", shape=[1], initializer=zero_init)

    return biases
def trainGAN():
    weights,biases = initialiseWeights(),initialiseBiases()
    z_vector = tf.placeholder(shape=[batch_size, z_size], dtype=tf.float32)
    x_vector = tf.placeholder(shape=[batch_size, 24, 24, 24, 1], dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            for start, end in zip(
                ...
                # for start,end in zip(
                #     range(0,len(polygonBatch[i]),y_height),
                #     range(batch_size,len(polygonBatch[i]),y_height)
                # ):
                #     print('-======--=-=-=-=-=-=-=')
                #     print(z_sample.shape)
                #     print(polygonBatch[start:end].shape)
                #     print('---=====--------------')
                next_polygon = polygonBatch[start:end].reshape(batch_size,24,24,24,1)
